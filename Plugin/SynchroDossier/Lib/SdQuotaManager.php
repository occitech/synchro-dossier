...
class SdQuotaManager implements CakeEventListener {
	...
	public function checkUploadAllowed($event) {
		...
		$messages = array(
			Configure::read('sd.Occitech.roleId') => __d('SynchroDossier', 'Il n\'y a plus de place pour ce dossier'),
			Configure::read('sd.Utilisateur.roleId') => __d('SynchroDossier', 'Désolé, l\'envoie de fichier n\'est actuellement pas disponible.'),
			Configure::read('sd.Admin.roleId') => __d('SynchroDossier', 'Le quota est atteint, vous devez contacter un SuperAdmin, ou supprimer des fichiers.'),
			Configure::read('sd.SuperAdmin.roleId') => __d('SynchroDossier', 'Le quota est atteint, vous devez commander plus de quota ou supprimer des fichiers.')
		);
